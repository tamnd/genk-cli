...
describes what users get. Pick a format with `-o`, or let genk choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
genk <command> -o table   # aligned columns for reading
genk <command> -o jsonl   # one JSON object per line, for piping
genk <command> -o json    # a single JSON array
genk <command> -o csv     # spreadsheet friendly
genk <command> -o tsv     # tab-separated
genk <command> -o url     # just the URL column
genk <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
genk <command> --fields id,title,url
```
...
```bash
genk <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
genk <command>            # a table, because this is a terminal
genk <command> | wc -l    # JSONL, because this is a pipe
```
